<section class="main">
    ...
    if(isset($_POST['newStatus'])){
        ...
        changeStatus($newStatus, $selectedRequestId);
    ...
    if(isset($_GET['p_id'])){
        $request_id = $_GET['p_id'];
    ?>
        <div class="anotherReservation">
            <ul>
    <?php
        // SELECT ALL DATA FROM REQUEST_ID
        $reservations = Reservation::find_by_id("request_id", $request_id);
        ...
        if($request->request_status === 'unread') {
            $newStatus = "pending";
            $status = new Status();
            $status->changeStatus($newStatus, $request_id);
    ...
    ?>
        ...
        <div class="mail">
            <div class="mail-header">
                <form action="" method="post" class="status <?= $reservation->request_status; ?>">
                    <select name="newStatus" class="status <?= $reservation->request_status; ?>" onchange="this.form.submit()">
                        <option value="unread" <?php $status->checkSelected("unread", $reservation->$request_status); ?>>Unread</option>
                        <option value="pending" <?php $status->checkSelected("pending", $reservation->$request_status); ?>>Pending</option>
                        <option value="confirmed" <?php $status->checkSelected("confirmed", $reservation->$request_status); ?>>Confirmed</option>
                        <option value="canceled" <?php $status->checkSelected("canceled", $reservation->$request_status); ?>>Canceled</option>
                    </select>
            ...
            <table class="mail-contents">
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="contents"><?php if($reservation->request_tel == "") { echo " - "; } else { echo $reservation->request_tel; }?></td>
                    </tr>
                    <tr>
                        <td class="label">Reservation Time :</td>
                        <td class="contents"><?= $reservation->request_time; ?></td>
                    </tr>
                    <tr>
